...
public class AS_ActionCamera : MonoBehaviour
{
    ...
    public event UnityAction BeginAction;
    public event UnityAction EndAction;

    private bool raisedEndEvent = false;
    ...
    {
        if (EndAction != null && !raisedEndEvent)
        {
            EndAction();
            raisedEndEvent = true;
        }
    ...
    public void RaiseBeginEvent()
    {
        if (BeginAction != null)
            BeginAction();
        raisedEndEvent = false;
    }
	...
	void cameraUpdate ()
	{
		...
		if (onWallCollision) {
			...
			this.transform.position = Vector3.Lerp (this.transform.position, cameraPosition + ((-this.transform.forward) * BaseDistance), 0.5f);
		}
			
		if (MainCamera) {
			MainCamera.fieldOfView = Mathf.Lerp (MainCamera.fieldOfView, FOVTarget, 0.5f * FOVspeed);	
		}
